"""
Lightweight AI Summarization module for NASA Space Biology.

This module provides fast, non-blocking text summarization designed to prevent
dashboard freezing while maintaining good quality summaries.
"""
...
try:
    ...
    log("⚠️ Using fallback summarization (no transformers)")
...
def load_summarizer(timeout: int = 20) -> Optional[object]:
    """
    Load lightweight summarizer with strict timeout to prevent blocking.
    
    Args:
        timeout: Maximum seconds to spend loading (default: 20s)
    
    Returns:
        Lightweight summarizer or None if failed/timeout
    """
    global _lightweight_summarizer, _load_failed
    
    # Return cached if available
    if _lightweight_summarizer is not None:
        return _lightweight_summarizer
    
    # Skip if previously failed to prevent repeated blocking
    if _load_failed:
        return None
    ...
    try:
        log("🚀 Loading lightweight summarizer...")
        start_time = time.time()
        
        # Use fastest, smallest model to prevent blocking
        _lightweight_summarizer = pipeline(
            "summarization",
            model="sshleifer/distilbart-cnn-6-6",  # Smallest BART variant
        ...
        if load_time > timeout:
            log(f"⚠️ Loading took {load_time:.1f}s, marking as failed")
            _load_failed = True
            _lightweight_summarizer = None
            return None
        
        log(f"✅ Lightweight summarizer loaded in {load_time:.1f}s")
        return _lightweight_summarizer
        
    except Exception as e:
        log_error(f"❌ Summarizer loading failed: {str(e)}")
        _load_failed = True
        _lightweight_summarizer = None
        return None


def extractive_summarize(text: str, max_length: int = 100) -> str:
    """
    Fast extractive summarization using sentence ranking.
    
    This never blocks and always returns a result quickly.
    """
    ...
    try:
        ...
        # Score sentences by space biology keywords
        keywords = {
            'high': ['significant', 'showed', 'found', 'results', 'effects', 'changes'],
            'biology': ['bone', 'muscle', 'heart', 'cell', 'tissue', 'health'],
            'space': ['microgravity', 'space', 'astronaut', 'mission', 'radiation'],
            'quantitative': [r'\d+%', r'\d+\s*fold', r'\d+\s*days?']
        }
        ...
        for i, sentence in enumerate(sentences):
            ...
            # Keyword scoring
            for term in keywords['high']:
                if term in sentence_lower:
                    score += 3
            for term in keywords['biology']:
                if term in sentence_lower:
                    score += 2
            for term in keywords['space']:
                if term in sentence_lower:
                    score += 2
            
            # Quantitative data bonus
            for pattern in keywords['quantitative']:
        ...
        return summary + "." if summary and not summary.endswith('.') else summary or clean_text[:max_length]
        
    except Exception as e:
        log_error(f"Extractive summarization failed: {str(e)}")
...
def summarize_text(
    ...
    """
    Fast, non-blocking text summarization with immediate fallback.
    
    Args:
        text: Input text to summarize
        min_length: Minimum summary length (default: 40)
        max_length: Maximum summary length (default: 100)
        timeout_seconds: Max time to spend (default: 5s)
        
    Returns:
        Summary text (always returns something)
    """
    if not text or len(text.strip()) < 30:
        return "Text too short for summarization"
    ...
    if summarizer:
        try:
            ...
            if time.time() - start_time > timeout_seconds:
                raise TimeoutError("AI summarization too slow")
            
            if result and len(result) > 0 and 'summary_text' in result[0]:
                summary = result[0]['summary_text'].strip()
                if len(summary) > 20 and len(summary) < len(text):
                    return summary
...
if __name__ == '__main__':
    test_text = "Space biology research investigates the effects of microgravity on living organisms. Studies show bone loss and muscle atrophy in astronauts."
    summary = summarize_text(test_text)
    print(f"Test summary: {summary}")
    
    # Test DataFrame
    test_df = pd.DataFrame({
        'abstract': [test_text, "Another space biology study about radiation effects on DNA."]
    })
    result_df = summarize_abstracts(test_df)
    print(f"DataFrame test: {len(result_df)} summaries generated")
...
def get_summarizer_info() -> Dict[str, Union[str, bool]]:
    ...
    return {
        ...
        'model_loaded': _lightweight_summarizer is not None,
        'model_name': 'distilbart-cnn-6-6' if TRANSFORMERS_AVAILABLE else 'extractive',
        'device': 'cpu',
...
def summarize_batch(texts: List[str], **kwargs) -> List[str]:
    """Backward compatibility for batch processing."""
    return [summarize_text(text, max_length=kwargs.get('max_length', 100)) for text in texts]


def clear_summarizer_cache():
    """Clear the cached summarizer to free memory."""
    global _lightweight_summarizer
    if _lightweight_summarizer is not None:
        del _lightweight_summarizer
        _lightweight_summarizer = None
        log("✓ Summarizer cache cleared")


def summarize_abstracts(df: pd.DataFrame, batch_size: int = 6, show_progress: bool = True, timeout: float = 30.0) -> pd.DataFrame:
    """
    Fast, non-blocking DataFrame summarization designed for real-time dashboards.
    ...
    log(f"� Fast summarizing {len(abstracts)} abstracts (timeout: {timeout}s)...")
    
    # Calculate per-abstract timeout to stay within limit
    per_abstract_timeout = min(3.0, timeout / max(len(abstracts), 1))
    
    summaries = []
    for i, abstract in enumerate(abstracts):
        if time.time() - start_time > timeout:
            log(f"⏰ Timeout reached at abstract {i+1}/{len(abstracts)}")
            # Fill remaining with extractive summaries
            for j in range(i, len(abstracts)):
                summaries.append(extractive_summarize(abstracts.iloc[j], 100))
            break
            
        if abstract and len(abstract.strip()) > 30:
            summary = summarize_text(abstract, max_length=100, timeout_seconds=per_abstract_timeout)
    ...
    log(f"✅ Fast summarization completed in {total_time:.1f}s ({len(summaries)/total_time:.1f} summaries/sec)")
